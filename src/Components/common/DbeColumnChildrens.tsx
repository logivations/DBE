...
export const getPatternRuleForFloats = (maxLength, fraction, columnName) => {
	...
	return <PatternRule key={`pattern-${columnName}`} pattern={pattern} message={translate('Data too long')} />;
};
...
export const getValidationForFilterName = (isNameExists) => {
	return [
		...
		<StringLengthRule
			max={60}
			message={translate('FILTER_NAME_SHOULD_CONTAIN_LESS_THAN_60_SYMBOLS')}
			key={'stringLengthRule-filterName'}
		/>,
		<CustomRule
			validationCallback={({ value }) => {
				const regexp = /((?<![\\])['"])/;
