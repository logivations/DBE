...
const CreateScreenBuilder = ({ configs, isScreenBuilder, tableInstanceManager }) => {
...
const ScreenBuilder = ({ isScreenBuilder }) => {
	...
	const { screenBuilderTables, screenBuilderTemplateID, screenBuilderTemplate } = useMemo(() => {
		return {
			screenBuilderTables: isScreenBuilder
				? screenBuilder.screenBuilderTables
				: [parentChildTable.childParentTable],
			screenBuilderTemplateID: isScreenBuilder
				? screenBuilder.screenBuilderTemplateID
				: parentChildTable.childParentTableTemplateID,
			screenBuilderTemplate: isScreenBuilder ? screenBuilder.splitElements : parentChildTable.childParentConfig,
		};
	}, [isScreenBuilder, screenBuilder, parentChildTable]);
		...
		if (screenBuilderTables.length && screenBuilderTemplateID) {
			Promise.all(
				screenBuilderTables.map((table_name, index) => {
					...
					return tableInstanceManager.createInstance(table_name, {
						...
						childParentTableJoinedColumnGroups: parentChildTable.childParentTableJoinedColumnGroups,
					});
				}),
			).then(() => setLoading(false));
		}
	}, [parentChildTable, screenBuilderTables, screenBuilderTemplateID]);
