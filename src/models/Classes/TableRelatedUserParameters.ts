...
class TableRelatedUserParameters {
	...
	public charts: Array<ChartDescription>;
	...
	public setDefaultHiddenColumnSetting(defaultHiddenColumnFilter): TableRelatedUserParameters {
		...
		const hiddenColumn = this.hiddenColumnSettings.find(
			({ settingName }) => settingName === defaultHiddenColumnSettingName,
		);
	...
	public editFilter(filterSetting: FilterSetting) {
		this.setSelectedFilter(filterSetting);
	...
	public editSortingSetting(sortSetting: SortSetting) {
		this.setSelectedSortOrder(sortSetting);
		return sortSetting;
	}
	...
	public deleteHiddenColumnsSetting(sortSetting: SortSetting) {
		this.setHiddenColumnSettings(
			this.hiddenColumnSettings.filter(({ settingId }) => sortSetting.settingId !== settingId),
		);
		return sortSetting;
	}
	...
	public editHiddenColumnsSetting(hiddenColumnSetting: HiddenColumnSetting) {
		this.setSelectedHiddenColumnFilter(hiddenColumnSetting);
	...
	public storeParentChildTableBuilderSettings(templateId: number, params, extraParams = {}) {
		return templateId ? { setting: { templateId, tableNames: params, ...extraParams } } : { setting: null };
	}
	...
	public saveSettings(params) {
		...
		this.intervalInSeconds = parseInt(intervalInSeconds, 10);
