...
import * as data from '../mockedData';
...
class Communicator extends BaseCommunicator {
	...
	public executeTableRelatedAction(actionId, params, tableName) {
		const queryParams = this.createQueryString({ actionId, tableName });
		return this.fetchBlobData(`${this.basePath}/executeTableRelatedAction`, queryParams, {
			method: 'POST',
			body: JSON.stringify(params),
			headers: { 'Content-Type': 'application/json' },
		});
	...
	public getSplitScreenSettings() {
		return Promise.resolve(data.splitScreenSettings);
	}

	public validateValueByCustomValidator(columnName, params, tableName) {
		return Promise.resolve(true);
		// const queryParams = this.createQueryString({ columnName, tableName });
	...
	public countSelectedRowsForTableAction(tableName) {
		const queryParams = this.createQueryString({ tableName });
		return this.fetchData(`${this.basePath}/countSelectedRowsForTableAction`, queryParams, {
			method: 'GET',
			headers: { 'Content-Type': 'application/json' },
		});
	}

	public getForeignKeyRelatedData(foreignKeyInfo, values, isParentTableHasWhId) {
		const queryParams = this.createQueryString({ warehouseId: isParentTableHasWhId ? this.warehouseId : null });
		return this.fetchData(`${this.basePath}/getForeignKeyRelatedData`, queryParams, {
			method: 'POST',
			body: JSON.stringify({ foreignKeyInfo, values }),
			headers: { 'Content-Type': 'application/json' },
		});
	}
