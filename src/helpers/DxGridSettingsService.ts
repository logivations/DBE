...
class DxGridSettings {
	...
	static parseSettingFullName(name: string) {
		const regExp = new RegExp(/([a-zA-Z]*)\[(\d*)]\.([a-zA-Z]*)/gm);
		const parsedData = name.matchAll(regExp).next().value;
		const [_, __, colIndex, fieldName] = parsedData ? [...parsedData.values()] : [];
		return [colIndex, fieldName];
	}
	...
	private static parseColumnsSetting(
		...
		const [colIndex, fieldName] = DxGridSettings.parseSettingFullName(settings.fullName);
		...
		if (fieldName === 'visible') {
			...
			tableInstance.toolbarPanelButtonManager.dispatchForToolbar({
				type: 'UPDATE_SELECTED_ITEM_KEY',
				payload: { selectedItemKey: null, buttonName: ButtonNames.hiddenColumnSettingButton },
			});
			tableInstance.toolbarPanelButtonManager.dispatchForToolbar({
				type: 'UPDATE_ITEMS',
				payload: { items: [], buttonName: ButtonNames.hiddenColumnSettingButton },
			});
		}
		if (
			!column ||
			fieldName === 'sortOrder' ||
			(fieldName === 'visible' && tableInstance.selectedHiddenColumnFilter)
		) {
			return;
		}
		...
		if (options instanceof Map) {
			const mapEntries: [string, { [p: string]: any; columnIndex: number }][] = [...options.entries()].reduce(
				(acc, [colName, settingObject]) => {
					...
					const col = getDbeDxGridInstance()
						.getVisibleColumns()
						.find(({ index }) => index === columnIndex);

					const extraParams = Object.keys(restSetting).reduce(
						(acc, key) => {
	...
	public onSettingsChanged(
		dxGridSettings: IDxGridSettings,
		getDbeDxGridInstance: TFunction,
		tableInstance: TableInstance,
	) {
	...
	public handleSettingChange(getDbeDxGridInstance: TFunction, tableInstance: TableInstance) {
		this.settingCache.forEach((dxGridSettings) => {
			const options = this.dxGridSettings.has(dxGridSettings.name)
				...
				: new Map();
			const parsedSettings = DxGridSettings.parseSettings(
				dxGridSettings,
				options,
	...
	public resetSettingsToDefault() {
		this.dxGridSettings.delete('columns');
