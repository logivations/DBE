...
function DbeEnumType<TBase extends DbeType>(BaseClass: TBase) {
	return class DbeEnum extends BaseClass {
		...
		public getChildren() {
			return [...this.rules, getLookupChildrenForColumn(this.getEnumDataSource(), this.getName())];
		}
		...
		public getEditorOptionsForFilterBuilder(value, onValueChangedHandler): any {
			return {
				...
				format: {
					formatter: (value) => {
						return this.getOptionByKey(parseInt(value, 10)).translatedAlias;
					},
				},
			};
		}
		...
		getDefaultValue() {
			const value = parseInt(this.defaultValue, 10);
			return isNaN(value) ? this.getEnumDataSource()[0]?.key : value;
		}
	};
}
