...
import {
	...
	getIntegerRule,
	getNumericRule,
	getPatternRuleForFloats,
} from '../../../Components/common/DbeColumnChildrens';
...
import { checkIfIntegerByFormat, isDefined, parseToDxFormat } from '../../../utils/utils';
...
function DbeNumericType<TBase extends DbeType>(BaseClass: TBase) {
	return class DbeNumeric extends BaseClass {
		...
		public getDxFormat() {
			const formatFromAbstractParameters =
				TimeFormats[this.getViewModel().getUnits().getAbstractParameters().format];
		...
		public getDefaultValue() {
			return isDefined(this.defaultValue) ? this.defaultValue.toString() : '';
		}
