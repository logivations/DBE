...
class FilterItem {
	...
	public parseValues(values, operator) {
		...
		switch(operator) {
			case DxOperations.CONTAINS: return parsedValues.map((val) => `%${val}%`);
			case DxOperations.STATS_WITH: return parsedValues.map((val) => `${val}%`);
			case DxOperations.ENDS_WITH: return parsedValues.map((val) => `%${val}`);
			default: return parsedValues;
		}
	}
	...
	public parseValuesToDxFilter() {
		const contains = new RegExp('^(%)(\\w*)(%)$', 'gm');
		const endsWith = new RegExp('^(%)(\\w*)$', 'gm');
		const startWith = new RegExp('^(\\w*)(%)$', 'gm');
