...
class TableDataModel implements ITableDataModel {
    ...
    public compareTableManager: CompareTableManager;
    ...
    constructor(private readonly metadata: Metadata) {
		this.compareTableManager = new CompareTableManager(this);
	}
	...
	public getCompareTableManager(): CompareTableManager {
		return this.compareTableManager;
	}
	...
	public getPrimaryKeys(): string[] {
		...
		if (!uniqueFields.length) {
			return this.getColumnsForAddingCloning().map((col) => col.getName());
		}
	...
	public async setForeignKeyData(tableName) {
		await communicator.getForeignKeyInfo(tableName).then((foreignKeyData) => {
			[...this.mappedColumnModelsByName.values()].map((column) => {
				if (column.getCellType() === CellType.FOREIGN_KEY_CELL) {
					return column.setForeignKeyData(foreignKeyData);
				}
			});
		});
	}
	...
	public prepareUniqueKeysData(rows: { [key: string]: any }[]) {
		...
		return (rows || []).reduce((acc, row) => {
			row && Object.entries(row).forEach(([name, value]) => {
	...
	public createFilterGroupFromUniqueKeys(rows: {[key: string]: any}[], uniqKeys?: {[key: string]: any}, operator?: string): FilterGroup {
		...
		return Object.entries(parsedUniqKeys).reduce((filterGroup: FilterGroup, [columnName, values]) => {
			filterGroup.addFilterCriteria(new FilterItem(columnName, operator || 'IN', uniq(values)));
			return filterGroup;
		}, new FilterGroup());
	}
}
