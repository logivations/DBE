...
const CompareTablesParametersModal = ({
	...
	const compareButtons = useMemo<ICompareDataButton[]>(() => {
		return tableInstance.tableDataModel.getCompareTableManager().getVisibleCompareTableButtons();
	}, []);
	const [compareType, setCompareType] = useState<ICompareDataButton>(() => compareButtons[0]);
	...
	const helpLink = useDbeActionsHelpLink(tableInstance.warehouseUiType, DbeActions[compareType.name]);
	return (
						...
						buttonOptions={{
							...
							onClick: () => {
								if (
									...
									handleCompareTable(compareTableDto, setCompareTableData, tableInstance.getTableName());
									closeModal();
