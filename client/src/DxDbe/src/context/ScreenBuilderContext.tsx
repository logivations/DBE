...
export const ScreenBuilderContextProvider = ({ children }: IContextProps) => {
    ...
    const setChildParentConfig = useCallback((childParentConfig, childParentTable, childParentTableTemplateID, joinedColumnGroups?) => {
        setParentChildTable(() => {
            return {
    ...
    const updateSplitElements = useCallback((tableToDelete, tableName, currenScreenBuilder) => {
        const updatedElements = deleteTable(currenScreenBuilder.splitElements, tableToDelete);
        const check = updatedElements.elements.length === 1
    ...
    useEffect(() => {
        communicator.getSplitScreenSettings().then((res) => {
            ...
            if (parentChildTableBuilderSettings.setting) {
                ...
                setChildParentConfig(template.splitScreenConfig(tablesName), tablesName, templateId);
            }
    ...
    return (
        <ScreenBuilderContext.Provider value={{
            parentChildTable,
